...
public class SdisGossip {
    ...
    public static void main(String[] args) {
        ...
        switch(args[2]) {
            ...
            case "push_blind" :
                ...
                break;
            case "pull_gossip" :
                new SdisGossip().pureGossipPull_sim(numNodes, numSims, k);
                break;
            case "pull_counter" :
                new SdisGossip().counterGossipPull_sim(numNodes, numSims, k);
                break;
            case "pull_blind" :
                new SdisGossip().blindRemovalPull_sim(numNodes, numSims, k);
                break;
        }
    ...
    public void blindRemovalPush_sim(int numNodes, int numSims, int maxk) {
      ...
      public void pureGossipPull_sim(int numNodes, int numSims, int maxk) {
        // The simualtion result vectors
        float[] s = new float[numSims];
        float[] m = new float[numSims];
        float[] t_avrg = new float[numSims];
        ...
        PrintWriter outputFile = null;
        try {
            outputFile = new PrintWriter("pureFeedback_Pull.csv", "US-ASCII");
        } catch (FileNotFoundException ex) {
            Logger.getLogger(SdisGossip.class.getName()).log(Level.SEVERE, null, ex);
        } catch (UnsupportedEncodingException ex) {
            Logger.getLogger(SdisGossip.class.getName()).log(Level.SEVERE, null, ex);
        ...
        for(int k=1; k <= maxk; k++) {
            ...
            System.out.println("Simulating Naive PUll Gossiping (k=" + k+")...");
    ...
    public void counterGossipPull_sim(int numNodes, int numSims, int maxk) {
        // The simualtion result vectors
        float[] s = new float[numSims];
        float[] m = new float[numSims];
        float[] t_avrg = new float[numSims];
        ...
        PrintWriter outputFile = null;
        try {
            outputFile = new PrintWriter("counterGossip_Pull.csv", "US-ASCII");
        } catch (FileNotFoundException ex) {
            Logger.getLogger(SdisGossip.class.getName()).log(Level.SEVERE, null, ex);
        } catch (UnsupportedEncodingException ex) {
            Logger.getLogger(SdisGossip.class.getName()).log(Level.SEVERE, null, ex);
        ...
        for(int k=1; k <= maxk; k++) {
            ...
            System.out.println("Simulating Counters Pull Gossiping (k=" + k+")...");
            ...
            for (int i=0; i < numSims; i++) {
                ...
                while(meshNetwork.infectiveNodes > 0) {
                    ...
                    // The nodes exchange messages
                    meshNetwork.pullCounterUpdates();
                ...
                // System.out.println("\ns      = " + meshNetwork.susceptibleNodes/1000.0 + "\ne^(-m) = " + Math.exp(-meshNetwork.trafficTotal/1000.0));
                // System.out.println("********************");
                if(meshNetwork.susceptibleNodes != 999) {
                    s[i] = (float)meshNetwork.susceptibleNodes/meshNetwork.numNodes;
                    m[i] = (float)meshNetwork.trafficTotal/meshNetwork.numNodes;
                    t_avrg[i] = meshNetwork.t_avrg;
                    t_last[i] = meshNetwork.t_last;
            ...
            outputFile.println(k + "," + (float)s_sum/numSims + "," + Math.sqrt(s_cov/(numNodes-1)) + "," +
                               (float)m_sum/numSims + "," + Math.sqrt(m_cov/(numNodes-1)) + "," + 
                               Math.exp(-(float)m_sum*m_sum*m_sum/numSims) + "," +
                               (float)t_avrg_sum/numSims + "," + Math.sqrt(t_avrg_cov/(numNodes-1)) + "," + 
                               (float)t_last_sum/numSims + "," + Math.sqrt(t_last_cov/(numNodes-1)) + "," +
                               (float)t_avrg_sum/t_last_sum);
        }
    ...
    public void blindRemovalPull_sim(int numNodes, int numSims, int maxk) {
        // The simualtion result vectors
        float[] s = new float[numSims];
        float[] m = new float[numSims];
        float[] t_avrg = new float[numSims];
        ...
        PrintWriter outputFile = null;
        try {
            outputFile = new PrintWriter("blindRemoval_Pull.csv", "US-ASCII");
        } catch (FileNotFoundException ex) {
            Logger.getLogger(SdisGossip.class.getName()).log(Level.SEVERE, null, ex);
        } catch (UnsupportedEncodingException ex) {
            Logger.getLogger(SdisGossip.class.getName()).log(Level.SEVERE, null, ex);
        ...
        for(int k=1; k <= maxk; k++) {
            ...
            System.out.println("Simulating Blind Removal Pull Gossiping (k=" + k+")...");
            ...
            for (int i=0; i < numSims; i++) {
                ...
                while(meshNetwork.infectiveNodes > 0) {
                    ...
                    // The nodes exchange messages
                    meshNetwork.pullBlindRemovalUpdates();
